    document.getElementById('tipoDocumento').addEventListener('change', function () {
        ...
        const labelExpedicao = document.querySelector('label[for="dataExpedicao"]') || document.querySelector('#dataExpedicao').previousElementSibling;

        if (tipo === 'CNH' || tipo === 'Passaporte') {
            labelExpedicao.textContent = 'Data de Vencimento:';
        } else {
            labelExpedicao.textContent = 'Data de Expedição:';
        }
    });

        document.getElementById('tipoDocumentoMae').addEventListener('change', function () {
        const tipo = this.value;
        const labelExpedicaoMae = document.querySelector('label[for="dataExpedicaoMae"]') || document.querySelector('#dataExpedicaoMae').previousElementSibling;

        if (tipo === 'CNH' || tipo === 'Passaporte') {
            labelExpedicaoMae.textContent = 'Data de Vencimento:';
        } else {
            labelExpedicaoMae.textContent = 'Data de Expedição:';
        }
    });

        document.getElementById('tipoDocumentoPai').addEventListener('change', function () {
        const tipo = this.value;
        const labelExpedicaoPai = document.querySelector('label[for="dataExpedicaoPai"]') || document.querySelector('#dataExpedicaoPai').previousElementSibling;

        if (tipo === 'CNH' || tipo === 'Passaporte') {
            labelExpedicaoPai.textContent = 'Data de Vencimento:';
        } else {
            labelExpedicaoPai.textContent = 'Data de Expedição:';
        }
    });

            document.getElementById('tipoDocumentoMenor').addEventListener('change', function () {
        const tipo = this.value;
        const labelExpedicaoMenor = document.querySelector('label[for="dataDocumentoMenor"]') || document.querySelector('#dataDocumentoMenor').previousElementSibling;

        if (tipo === 'CNH' || tipo === 'Passaporte') {
            labelExpedicaoMenor.textContent = 'Data de Vencimento:';
        } else {
            labelExpedicaoMenor.textContent = 'Data de Expedição:';
        }
    });
...
function gerarDocumento() {
    ...
    if (tipoProcesso === "transcricao") {
        textoProcura = `<strong>${nome}</strong>, ${nacionalidade}, no estado civil de ${estadoCivil}, ${profissao}, nascid${generoLetra} em ${dataNascimento}, na cidade de ${cidadeNascimento} – ${ufNascimento}, residente em ${residencia}, portador${genero === "homem" ? "" : "a"} do documento de identificação (${tipoDocumento}) nº ${documento}, ${tipoDocumento === "RG" ? "expedido em" : "com data de vencimento em"} ${dataExpedicao} pelo órgão ${orgaoExpedidor}, constitui como sua bastante procuradora a <strong>Dra. CARLA OSSUNA</strong>, advogada inscrita na Ordem dos Advogados de Portugal, com cédula profissional sob o Nº 64201L, com morada profissional na Rua Febo Muniz, 27B, Edifício LACS Anjos - Código Postal 1150-152, Lisboa - Portugal, <strong>a quem confere poderes especiais e necessários para ${generoLetra} representar perante a Conservatória dos Registos Centrais e/ou Conservatória dos Registos Civis e suas extensões, para proceder à Transcrição de Casamento ocorrido no estrangeiro entre ${nubente1} e ${nubente2}</strong>${portugues !== "proprio" ? `, de meus ${portugues},` : ""} podendo para o feito declarar, praticar e assinar tudo o que seja necessário ao indicado fim, inclusive podendo desistir do pedido, prestar declarações e substabelecer os poderes que lhe foram conferidos.`
    }

    if (tipoProcesso === "filhosMenores" && tipoProcuracao === "pm") {
        textoProcura = `<strong>${nomeMaeMenor}</strong>, ${nacionalidadeMae}, no estado civil de ${estadoCivilMae}, ${profissaoMae}, com residência habitual em ${residenciaMae}, portadora do documento de identificação (${tipoDocumentoMae}) n.º ${documentoMae}, com data de ${tipoDocumentoMae === "RG" ? "expecição" : "vencimento"} em ${dataExpedicaoMae} pelo órgão emissor ${orgaoExpedidorMae}, e <br>
<strong>${nomePaiMenor}</strong>, ${nacionalidadePai}, no estado civil de ${estadoCivilPai}, ${profissaoPai}, com residência habitual em ${residenciaPai}, portadora do documento de identificação (${tipoDocumentoPai}) n.º ${documentoPai}, com data de ${tipoDocumentoPai === "RG" ? "expecição" : "vencimento"} em ${dataExpedicaoPai} pelo órgão emissor ${orgaoExpedidorPai}, constitui como sua bastante procuradora a <strong>Dra. CARLA OSSUNA</strong>, advogada inscrita na Ordem dos Advogados de Portugal, com cédula profissional sob o Nº 64201L, com morada profissional na Rua Febo Muniz, 27B, Edifício LACS Anjos - Código Postal 1150-152, Lisboa - Portugal, a quem confere os poderes especiais e necessários para os  representar perante a Conservatória dos Registos Centrais de Lisboa/Arquivo Distrital do Porto, ao abrigo do art. 1º, nº 1, al.c) da Lei da Nacionalidade nº 37/81, de 3 de Outubro, com sua posterior alteração pela Lei Orgânica n.º 2/2020, requerer a atribuição da Nacionalidade Portuguesa do nosso filh${generoLetra} ${nomeMenor}, nascid${generoLetra} em ${dataNascimentoMenor}, na cidade de ${cidadeNascimentoMenor}, no estado de ${estadoNascimento}, portador${genero === "homem" ? "" : "a"} do documento de identificação (${tipoDocumentoMenor}) n.° ${documentoMenor}, com data de ${tipoDocumentoMenor === "RG" ? "expecição" : "vencimento"} em ${dataDocumentoMenor} pelo órgão ${orgaoExpedidorMenor},  e que seja lavrado o respetivo registo, por ser filh${generoLetra} de filh${generoLetra} de <strong> ${portugues} ${portugues === "pai" ? "português" : "portuguesa"}</strong>, promovendo, se necessário, a inscrição do respetivo nascimento, fixação do nome, praticando e assinando tudo o que seja necessário ao indicado fim, podendo, prestar declarações e substabelecer os poderes que lhe foram conferidos. `;
    }
